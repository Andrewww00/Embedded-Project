...
void start_mag_cal(void)
{
    ...
    do {
        ...
        magCalInput.mag[0] = mag_data.x * 0.15f / 50.0f;
        magCalInput.mag[1] = mag_data.y * 0.15f / 50.0f;
        magCalInput.mag[2] = mag_data.z * 0.15f / 50.0f;
...
int main(void) {
    ...
    HAL_Delay(2000);   // board ferma sul tavolo
    ...
    while (1) {
    	...
    	float delta_time = (now - last_tick) * 0.001f;
    	last_tick = now;
